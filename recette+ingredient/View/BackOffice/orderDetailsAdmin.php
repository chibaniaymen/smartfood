...
foreach ($controllerFiles as $class => $path) { if (file_exists($path)) require_once $path; }
$legacyBroken = !class_exists('MarathonController') || !class_exists('ParcoursController') || !class_exists('CommandeController') || !class_exists('LigneCommandeController');
if ($legacyBroken) {
    ...
    echo '<p>Certaines fonctionnalités ont été retirées du projet. Le backoffice doit être adapté.</p>';
...
if ($viewCommande) {
    ...
    <head>
        ...
        <title>Détails commande #<?php echo htmlspecialchars($commande['idcommande']); ?> - Recettes (Admin)</title>
        <style>
