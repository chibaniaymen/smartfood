...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($result['success']) {
        header('Location: ../../index.php?success=article_added');
        exit;
    }
...
<head>
  ...
  <title>Publier un article — SmartFood</title>
  ...
  <style>
    ...
    .btn-green { background: #2D6A4F; color: white; border-radius: 50px; padding: 12px 30px; border: none; font-weight: 700; transition: .3s; }
    .btn-green:hover { background: #1b4332; transform: translateY(-2px); }
    ...
    .form-control:focus { border-color: #2D6A4F; box-shadow: none; }
  </style>
</head>
<body>
...
<div class="container">
  <div class="row justify-content-center">
    <div class="col-lg-7">
      <div class="form-box">
        <h2 class="text-center mb-4" style="font-weight:800;">✍️ Publier un article</h2>
        <p class="text-center text-muted mb-4">Partagez vos connaissances nutritionnelles avec la communauté.</p>

        <?php if ($error): ?>
          <div class="alert alert-danger" style="border-radius:12px;">❌ <?php echo htmlspecialchars($error); ?></div>
        <?php endif; ?>

        <form method="POST" id="userAddForm" novalidate onsubmit="return validateAndSubmit();">
          <div class="mb-4">
            <label class="form-label">Titre de l'article</label>
            <input type="text" class="form-control" id="title" name="title" placeholder="Ex: Les bienfaits du magnésium" value="<?php echo htmlspecialchars($formData['title']); ?>">
            <div class="text-danger small mt-1" id="err-title" style="display:none;"></div>
          </div>

          <div class="mb-4">
            <label class="form-label">Contenu</label>
            <textarea class="form-control" id="content" name="content" rows="8" placeholder="Rédigez votre texte ici..."><?php echo htmlspecialchars($formData['content']); ?></textarea>
            <div class="text-danger small mt-1" id="err-content" style="display:none;"></div>
          </div>

          <div class="d-flex gap-3 justify-content-center mt-4">
            <button type="submit" class="btn-green">Publier maintenant</button>
...
function validateAndSubmit() {
  ...
  if (ok) document.getElementById('userAddForm').submit();
}
